...
path_geopackge_v2 = base_directory.joinpath("LibroCoobook", "0_package")
        ...
        for layer in layers:
            ...
            for entity in modelspace.query(f'*[layer=="{layer.dxf.name}"]'):
                ...
                if entity.dxftype() == 'POINT':
                    geom = Point(entity.dxf.location)
                elif entity.dxftype() == 'LINE':
                    geom = LineString([(p.x, p.y) for p in entity.vertices])
                elif entity.dxftype() == 'LWPOLYLINE':
                    geom = Polygon([(p.x, p.y) for p in entity.vertices])
                elif entity.dxftype() == 'MULTIPOINT':
                    for p in entity.points:
                        features.append({'geometry': Point(p.x, p.y), **entity.dxf.attribs})
                    continue

                if geom:
                    features.append({'geometry': geom, **entity.dxf.attribs})
            ...
            if features:
                ...
                gdf.to_file(os.path.join(str(path_geopackge_v2), f'{layer.dxf.name}.shp'), driver='ESRI Shapefile')
        return
        """
                shapefile_path = f'{layer.dxf.name}.shp'
                print(shapefile_path)
                
                # Crear un shapefile para cada capa
                writer = QgsVectorFileWriter(
                    os.path.join(path_geopackge_v2, shapefile_path),
                    'UTF-8',
                    lyr.fields(),
                    lyr.wkbType(),
                    crs,
                ...
                # Añadir características al shapefile
                writer.addFeature(lyr.getFeatures())
                
                # Finalizar el proceso de escritura
                del writer
        """
        return
        # Abrir el archivo DXF
        layer = QgsVectorLayer(str(file_path), 'DXF Layer', 'ogr')
        if not layer.isValid():
            print('Capa no válida.')
        else:
            print("Se muestra capa")
            # Recorrer las capas y guardar cada una como un shapefile
            for feature in layer:
                print(feature.name)
            return 
            for feature in layer.getFeatures():
                # Nombre del shapefile de salida basado en el nombre de la capa y el ID de la característica
                shapefile_path = f'{layer.name()}_{feature.id()}.shp'
                print(shapefile_path)
